...
@section('meta_description', __('Privacy Policy'))

@section('content')
    @php
        $page = class_exists(\App\Models\Page::class)
            ? \App\Models\Page::where('slug', 'privacy')
                ->where('locale', app()->getLocale())
                ->where('is_published', true)
                ->first()
            : null;
        $pageTitle = $page ? (is_array($page->title) ? ($page->title[app()->getLocale()] ?? reset($page->title)) : ($page->title ?? __('Privacy Policy'))) : __('Privacy Policy');
    @endphp
            ...
            <div class="row wow fadeInUp" data-wow-delay=".3s">
                <div class="col-xl-12">
                    <div class="blog-details-wrapper">
                        ...
                        @else
                            <div class="blog-content">
                                <p>{{ __('We respect your privacy and handle your data with care...') }}</p>
                            </div>
                        @endif
                    </div>
                </div>
