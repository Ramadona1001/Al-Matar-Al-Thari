...
(function($) {
  ...
  function initForms() {
    ...
    $('#registerForm').on('submit', function(e) {
      ...
      if (!fullName) {
        showFieldError('#fullName', window.t ? window.t('common.required') : 'This field is required');
        isValid = false;
      }
      
      // Validate email
      const email = form.find('#registerEmail').val().trim();
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!email) {
        showFieldError('#registerEmail', window.t ? window.t('common.required') : 'This field is required');
        isValid = false;
      } else if (!emailRegex.test(email)) {
        showFieldError('#registerEmail', window.t ? window.t('common.invalidEmail') : 'Please enter a valid email');
        isValid = false;
      }
      ...
      if (!phone) {
        showFieldError('#phone', window.t ? window.t('common.required') : 'This field is required');
        isValid = false;
      }
      ...
      if (!password) {
        showFieldError('#registerPassword', window.t ? window.t('common.required') : 'This field is required');
        isValid = false;
      ...
      if (!confirmPassword) {
        showFieldError('#confirmPassword', window.t ? window.t('common.required') : 'This field is required');
        isValid = false;
      } else if (password !== confirmPassword) {
        showFieldError('#confirmPassword', window.t ? window.t('common.passwordMismatch') : 'Passwords do not match');
        isValid = false;
      }
      
      if (isValid) {
        // Show success message (in real app, this would submit to server)
        showSuccessMessage(form, window.t ? window.t('common.success') : 'Registration successful!');
        form[0].reset();
      }
    });
