    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Create FAQ') }}</h5>
            </div>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.faqs.store') }}">
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category') }}" class="form-control" list="categories">
                    <datalist id="categories">
                        @foreach($categories ?? [] as $cat)
                            <option value="{{ $cat }}">
                        @endforeach
                    </datalist>
                    <small class="text-muted">{{ __('Optional category for grouping FAQs') }}</small>
                </div>
                <div class="col-md-3">
                                    ...
                                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="faq-{{ $locale }}" role="tabpanel">
                                        <div class="row g-3">
                                            <div class="col-12">
                                                ...
                                                <input type="text" name="question_{{ $locale }}" id="question_{{ $locale }}" value="{{ old("question_$locale") }}" class="form-control @error("question_$locale") is-invalid @enderror" required>
                                            ...
                                            <div class="col-12">
                                                ...
                                                <textarea name="answer_{{ $locale }}" id="answer_{{ $locale }}" rows="6" class="form-control @error("answer_$locale") is-invalid @enderror" required>{{ old("answer_$locale") }}</textarea>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Create FAQ') }}</button>
            </div>
        </form>
    </div>
</div>
