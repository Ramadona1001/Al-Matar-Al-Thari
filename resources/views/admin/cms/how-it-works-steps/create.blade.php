...
@section('title', __('Create How It Works Step'))
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Create How It Works Step') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="POST" action="{{ route('admin.how-it-works-steps.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="row g-4">
                <div class="col-md-4">
                    <label for="step_number" class="form-label fw-semibold">{{ __('Step Number') }} <span class="text-danger">*</span></label>
                    <input type="number" name="step_number" id="step_number" value="{{ old('step_number', $nextStepNumber ?? 1) }}" class="form-control @error('step_number') is-invalid @enderror" min="1" required>
                    @error('step_number')
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="icon" id="icon" value="{{ old('icon') }}" class="form-control" placeholder="fas fa-check">
                    <small class="text-muted">{{ __('Font Awesome icon class (e.g., fas fa-check)') }}</small>
                </div>
                <div class="col-md-4">
                    ...
                    <small class="text-muted">{{ __('Optional: Upload an image instead of using icon') }}</small>
                </div>
                <div class="col-md-3">
                                    ...
                                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="step-{{ $locale }}" role="tabpanel">
                                        <div class="row g-3">
                                            <div class="col-12">
                                                ...
                                                <input type="text" name="title_{{ $locale }}" id="title_{{ $locale }}" value="{{ old("title_$locale") }}" class="form-control @error("title_$locale") is-invalid @enderror" required>
                                                @error("title_$locale")
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            <div class="col-12">
                                                ...
                                                <textarea name="description_{{ $locale }}" id="description_{{ $locale }}" rows="4" class="form-control @error("description_$locale") is-invalid @enderror" required>{{ old("description_$locale") }}</textarea>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Create Step') }}</button>
            </div>
        </form>
    </div>
</div>
