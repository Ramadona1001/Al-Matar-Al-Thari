...
@section('title', __('How It Works Steps Management'))

@section('breadcrumb')
    <li class="breadcrumb-item active" aria-current="page">{{ __('How It Works Steps') }}</li>
@endsection

@section('actions')
    <a href="{{ route('admin.how-it-works-steps.create') }}" class="btn btn-primary btn-animated">
        <i class="fas fa-plus me-2"></i>{{ __('Create Step') }}
    </a>
@endsection
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Search & Filter Steps') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="GET" action="{{ route('admin.how-it-works-steps.index') }}" class="row g-3">
            <div class="col-md-4">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <h5 class="fw-bold mb-0 text-gray-900">{{ __('How It Works Steps') }}</h5>
                </div>
            </div>
            <div class="text-muted small">
                <i class="fas fa-info-circle me-1"></i>
                {{ __('Total') }}: <strong>{{ $steps->count() }}</strong>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <div class="table-responsive-modern">
            <table class="table table-modern align-middle mb-0" id="stepsTable" data-dt-init="false">
                <thead class="table-header-modern">
                    <tr>
                        <th class="fw-semibold text-gray-700">{{ __('Step #') }}</th>
                        <th class="fw-semibold text-gray-700">{{ __('Icon/Image') }}</th>
                        ...
                        <tr class="table-row-modern">
                            ...
                            <td>
                                @if($step->image_path)
                                    <img src="{{ asset('storage/' . $step->image_path) }}" 
                                         alt="{{ $step->translate(app()->getLocale())->title ?? '' }}" 
                                         class="rounded" 
                                         style="width: 60px; height: 60px; object-fit: cover;">
                                @elseif($step->icon)
                                    <div class="bg-light rounded d-flex align-items-center justify-content-center" style="width: 60px; height: 60px;">
                                        <i class="{{ $step->icon }} text-primary fs-4"></i>
                                    </div>
                                @else
                                    <div class="bg-light rounded d-flex align-items-center justify-content-center" style="width: 60px; height: 60px;">
                                        <i class="fas fa-step-forward text-muted"></i>
                                    </div>
                                @endif
                            </td>
                            <td>
                                @php
                                    $currentTranslation = $step->translate(app()->getLocale());
                                    $enTranslation = $step->translate('en');
                                ...
                                @php
                                    $description = ($currentTranslation && $currentTranslation->description) ? $currentTranslation->description : (($enTranslation && $enTranslation->description) ? $enTranslation->description : null);
                                @endphp
                                @if($description)
                                    <small class="text-muted d-block mt-1">{{ Str::limit($description, 60) }}</small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('admin.how-it-works-steps.edit', $step) }}" 
                                       class="btn btn-sm btn-outline-primary btn-animated" 
                                       title="{{ __('Edit') }}">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.how-it-works-steps.destroy', $step) }}" 
                                          method="post" 
                                          class="d-inline" 
                                          onsubmit="return confirm('{{ __('Are you sure you want to delete this step?') }}')">
                                        @csrf 
                                        @method('DELETE')
                                        <button type="submit" 
