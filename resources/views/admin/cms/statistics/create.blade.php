...
@section('title', __('Create Statistic'))
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Create Statistic') }}</h5>
            </div>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.statistics.store') }}">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="suffix" id="suffix" value="{{ old('suffix') }}" class="form-control" placeholder="e.g., +, %, K, M">
                </div>
                <div class="col-md-6">
                    <label for="icon" class="form-label fw-semibold">{{ __('Icon') }}</label>
                    <input type="text" name="icon" id="icon" value="{{ old('icon') }}" class="form-control" placeholder="fas fa-users">
                                    ...
                                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="statistic-{{ $locale }}" role="tabpanel">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <textarea name="description_{{ $locale }}" id="description_{{ $locale }}" rows="3" class="form-control">{{ old("description_{$locale}") }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Create Statistic') }}</button>
            </div>
        </form>
    </div>
</div>
