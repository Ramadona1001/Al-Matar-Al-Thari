...
@section('actions')
    <a href="{{ route('admin.statistics.create') }}" class="btn btn-primary btn-animated">
        <i class="fas fa-plus me-2"></i>{{ __('Create Statistic') }}
    </a>
@endsection
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Search & Filter Statistics') }}</h5>
            </div>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('admin.statistics.index') }}" class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" id="search" class="form-control" 
                       ...
                       placeholder="{{ __('Search by label or value...') }}">
            </div>
            <div class="col-12">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <h5 class="fw-bold mb-0 text-gray-900">{{ __('Statistics') }}</h5>
                </div>
            </div>
            <div class="text-muted small">
                <i class="fas fa-info-circle me-1"></i>
                {{ __('Total') }}: <strong>{{ $statistics->count() }}</strong>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <div class="table-responsive-modern">
            <table class="table table-modern align-middle mb-0" id="statisticsTable" data-dt-init="false">
                <thead class="table-header-modern">
                    <tr>
                        <th class="fw-semibold text-gray-700">{{ __('Icon') }}</th>
                        <th class="fw-semibold text-gray-700">{{ __('Label') }}</th>
                        <th class="fw-semibold text-gray-700">{{ __('Value') }}</th>
                ...
                <tbody>
                    @foreach($statistics as $statistic)
                        <tr class="table-row-modern">
                            <td>
                                @if($statistic->icon)
                                    <i class="{{ $statistic->icon }} text-primary" style="font-size: 1.5rem;"></i>
                                @else
                                    <i class="fas fa-chart-bar text-muted" style="font-size: 1.5rem;"></i>
                                @endif
                            </td>
                            <td>
                                <div class="fw-semibold text-gray-900">{{ $statistic->label }}</div>
                                @if($statistic->description)
                                    <small class="text-muted d-block mt-1">{{ \Illuminate\Support\Str::limit($statistic->description, 50) }}</small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                <span class="badge bg-primary-subtle text-primary rounded-pill px-3 py-1 fw-semibold">
                                    {{ strtoupper($statistic->locale ?? 'en') }}
                                </span>
                            </td>
                            <td>
...
(function() {
    function initTable() {
        ...
        const $table = jQuery('#statisticsTable');
        ...
        const options = {
            ...
            emptyTable: '<div class="empty-state-modern text-center py-5"><div class="empty-icon-wrapper mb-3"><i class="fas fa-chart-line"></i></div><p class="text-muted mb-0 fw-semibold">{{ __('No statistics found') }}</p><a href="{{ route('admin.statistics.create') }}" class="btn btn-primary btn-sm mt-3"><i class="fas fa-plus me-2"></i>{{ __('Create Your First Statistic') }}</a></div>'
        };
