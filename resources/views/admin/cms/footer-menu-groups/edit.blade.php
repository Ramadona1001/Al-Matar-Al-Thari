...
@section('title', __('Edit Footer Menu Group'))
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Edit Footer Menu Group') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="POST" action="{{ route('admin.footer-menu-groups.update', $footerMenuGroup) }}">
            @csrf
            @method('PUT')
            <div class="row g-4">
                <div class="col-md-6">
                    <label for="order" class="form-label fw-semibold">{{ __('Order') }}</label>
                    <input type="number" name="order" id="order" value="{{ old('order', $footerMenuGroup->order ?? 0) }}" class="form-control" min="0">
                    <small class="text-muted">{{ __('Display order in footer') }}</small>
                </div>
                <div class="col-md-6 d-flex align-items-end">
                    <div class="form-check form-switch">
                        <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ old('is_active', $footerMenuGroup->is_active) ? 'checked' : '' }}>
                                ...
                                @foreach($locales as $index => $locale)
                                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="group-{{ $locale }}" role="tabpanel">
                                        <div class="row g-3">
                                            <div class="col-12">
                                                @php
                                                    $translation = $footerMenuGroup->translate($locale);
                                                    $nameValue = old("name_$locale", $translation ? $translation->name : '');
                                                @endphp
                                                <label for="name_{{ $locale }}" class="form-label fw-semibold">{{ __('Group Name') }} ({{ strtoupper($locale) }}) <span class="text-danger">*</span></label>
                                                <input type="text" name="name_{{ $locale }}" id="name_{{ $locale }}" value="{{ $nameValue }}" class="form-control @error("name_$locale") is-invalid @enderror" required placeholder="{{ __('e.g., Quick Links, Company, Support') }}">
                                                @error("name_$locale")
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Update Footer Menu Group') }}</button>
            </div>
        </form>
    </div>
</div>
