...
@section('title', __('Footer Menu Groups Management'))

@section('breadcrumb')
    <li class="breadcrumb-item active" aria-current="page">{{ __('Footer Menu Groups') }}</li>
@endsection

@section('actions')
    <a href="{{ route('admin.footer-menu-groups.create') }}" class="btn btn-primary btn-animated">
        <i class="fas fa-plus me-2"></i>{{ __('Create Footer Menu Group') }}
    </a>
@endsection
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Search & Filter Footer Menu Groups') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="GET" action="{{ route('admin.footer-menu-groups.index') }}" class="row g-3">
            <div class="col-md-4">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <h5 class="fw-bold mb-0 text-gray-900">{{ __('Footer Menu Groups') }}</h5>
                </div>
            </div>
            <div class="text-muted small">
                <i class="fas fa-info-circle me-1"></i>
                {{ __('Total') }}: <strong>{{ $groups->count() }}</strong>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        @if($groups->count() > 0)
            <div class="table-responsive-modern">
                <table class="table table-modern align-middle mb-0" id="footerMenuGroupsTable" data-dt-init="false">
                    <thead class="table-header-modern">
                        <tr>
                            <th class="fw-semibold text-gray-700">{{ __('Name') }}</th>
                            <th class="fw-semibold text-gray-700">{{ __('Menu Items') }}</th>
                        ...
                        @foreach($groups as $group)
                            <tr class="table-row-modern">
                                <td>
                                    ...
                                    @endphp
                                    <div class="fw-semibold text-gray-900">{{ $name }}</div>
                                </td>
                                <td>
                                    ...
                                    <a href="{{ route('admin.menus.index', ['name' => 'footer', 'group' => $group->id]) }}" 
                                       class="btn btn-sm btn-link text-primary ms-2">
                                        <i class="fas fa-edit"></i> {{ __('Manage') }}
                                    </a>
                                </td>
                                <td>
                                ...
                                <td class="text-end">
                                    <div class="btn-group" role="group">
                                        ...
                                        <form action="{{ route('admin.footer-menu-groups.destroy', $group) }}" 
                                              method="post" 
                                              class="d-inline" 
                                              onsubmit="return confirm('{{ __('Are you sure you want to delete this footer menu group?') }}')">
                                            @csrf 
                                            @method('DELETE')
                                            <button type="submit" 
        ...
        @else
            <div class="text-center py-5">
                <div class="text-muted">
                    ...
                    <p>{{ __('No footer menu groups found.') }}</p>
                    <a href="{{ route('admin.footer-menu-groups.create') }}" class="btn btn-primary btn-animated">
                        <i class="fas fa-plus me-2"></i>{{ __('Create First Group') }}
                    </a>
                </div>
            </div>
        @endif
...
(function() {
    function initTable() {
        ...
        const $table = jQuery('#footerMenuGroupsTable');
        if ($.fn.DataTable.isDataTable('#footerMenuGroupsTable')) {
