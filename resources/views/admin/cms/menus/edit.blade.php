    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Edit Menu Item') }}</h5>
            </div>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.menus.update', $menu) }}">
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <select name="name" id="name" class="form-select @error('name') is-invalid @enderror" required>
                        @foreach($menuNames ?? [] as $menuName)
                            <option value="{{ $menuName }}" {{ old('name', $menu->name) == $menuName ? 'selected' : '' }}>{{ ucfirst($menuName) }}</option>
                        @endforeach
                    </select>
                            ...
                            <div class="tab-content">
                                @foreach($locales as $index => $locale)
                                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="label-{{ $locale }}" role="tabpanel">
                                        ...
                                        <input type="text" name="label_{{ $locale }}" id="label_{{ $locale }}" value="{{ old("label_{$locale}", $menu->translate($locale)->label ?? '') }}" class="form-control @error("label_{$locale}") is-invalid @enderror" required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="url" id="url" value="{{ old('url', $menu->url) }}" class="form-control" placeholder="/page or https://example.com">
                </div>
                <div class="col-md-6">
                    <label for="route" class="form-label fw-semibold">{{ __('Route Name') }}</label>
                    <input type="text" name="route" id="route" value="{{ old('route', $menu->route) }}" class="form-control" placeholder="public.home">
                </div>
                <div class="col-md-6">
                    <label for="icon" class="form-label fw-semibold">{{ __('Icon') }}</label>
                    <input type="text" name="icon" id="icon" value="{{ old('icon', $menu->icon) }}" class="form-control" placeholder="fas fa-home">
                </div>
                <div class="col-md-6">
                    <label for="order" class="form-label fw-semibold">{{ __('Order') }}</label>
                    <input type="number" name="order" id="order" value="{{ old('order', $menu->order) }}" class="form-control" min="0">
                </div>
                ...
                <div class="col-md-6 d-flex align-items-end">
                    <div class="form-check form-switch">
                        <input type="checkbox" class="form-check-input" id="open_in_new_tab" name="open_in_new_tab" value="1" {{ old('open_in_new_tab', $menu->open_in_new_tab) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Update Menu Item') }}</button>
            </div>
        </form>
    </div>
</div>
