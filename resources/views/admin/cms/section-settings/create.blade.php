...
@section('title', __('Create Section Setting'))
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Create Section Setting') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="POST" action="{{ route('admin.section-settings.store') }}">
            @csrf
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <select name="section_key" id="section_key" class="form-select @error('section_key') is-invalid @enderror" required>
                        ...
                        @foreach($sectionKeys as $key)
                            <option value="{{ $key }}" {{ old('section_key') == $key ? 'selected' : '' }}>{{ $key }}</option>
                        @endforeach
                    </select>
                    @error('section_key')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">{{ __('Unique identifier for this section setting') }}</small>
                </div>
                <div class="col-md-3">
                                    ...
                                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="setting-{{ $locale }}" role="tabpanel">
                                        <div class="row g-3">
                                            <div class="col-12">
                                                ...
                                                <input type="text" name="title[{{ $locale }}]" id="title_{{ $locale }}" value="{{ old("title.$locale") }}" class="form-control">
                                            </div>
                                            <div class="col-12">
                                                <label for="subtitle_{{ $locale }}" class="form-label fw-semibold">{{ __('Subtitle') }} ({{ strtoupper($locale) }})</label>
                                                <textarea name="subtitle[{{ $locale }}]" id="subtitle_{{ $locale }}" rows="3" class="form-control">{{ old("subtitle.$locale") }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                ...
                <div class="col-12">
                    ...
                    <textarea name="options" id="options" rows="5" class="form-control" placeholder='{"key": "value"}'>{{ old('options') ? json_encode(old('options'), JSON_PRETTY_PRINT) : '' }}</textarea>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Create Section Setting') }}</button>
            </div>
        </form>
    </div>
</div>
