...
@section('title', __('Section Settings Management'))

@section('breadcrumb')
    <li class="breadcrumb-item active" aria-current="page">{{ __('Section Settings') }}</li>
@endsection

@section('actions')
    <a href="{{ route('admin.section-settings.create') }}" class="btn btn-primary btn-animated">
        <i class="fas fa-plus me-2"></i>{{ __('Create Section Setting') }}
    </a>
@endsection
    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Search & Filter Section Settings') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="GET" action="{{ route('admin.section-settings.index') }}" class="row g-3">
            <div class="col-md-4">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="search" id="search" class="form-control" 
                       ...
                       placeholder="{{ __('Search by section key or title...') }}">
            </div>
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <h5 class="fw-bold mb-0 text-gray-900">{{ __('Section Settings') }}</h5>
                </div>
            </div>
            <div class="text-muted small">
                <i class="fas fa-info-circle me-1"></i>
                {{ __('Total') }}: <strong>{{ $settings->count() }}</strong>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <div class="table-responsive-modern">
            <table class="table table-modern align-middle mb-0" id="sectionSettingsTable" data-dt-init="false">
                <thead class="table-header-modern">
                    <tr>
                        <th class="fw-semibold text-gray-700">{{ __('Section Key') }}</th>
                ...
                <tbody>
                    @foreach($settings as $setting)
                        <tr class="table-row-modern">
                            <td>
                                <code class="text-primary fw-semibold">{{ $setting->section_key }}</code>
                            </td>
                            <td>
                                @php
                                    $currentTranslation = $setting->translate(app()->getLocale());
                                    $enTranslation = $setting->translate('en');
                            ...
                            <td class="text-end">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('admin.section-settings.edit', $setting) }}" 
                                       class="btn btn-sm btn-outline-primary btn-animated" 
                                       title="{{ __('Edit') }}">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.section-settings.destroy', $setting) }}" 
                                          method="post" 
                                          class="d-inline" 
                                          onsubmit="return confirm('{{ __('Are you sure you want to delete this section setting?') }}')">
                                        @csrf 
                                        @method('DELETE')
                                        <button type="submit" 
...
(function() {
    function initTable() {
        ...
        const $table = jQuery('#sectionSettingsTable');
        if ($.fn.DataTable.isDataTable('#sectionSettingsTable')) {
