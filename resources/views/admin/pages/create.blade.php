    ...
    <div class="card-header bg-white border-0 pb-0 pt-4 px-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                ...
                <h5 class="fw-bold mb-0 text-gray-900">{{ __('Create Page') }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="POST" action="{{ route('admin.pages.store') }}" enctype="multipart/form-data">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <select name="locale" id="locale" class="form-select @error('locale') is-invalid @enderror" required>
                        ...
                        @foreach($locales as $loc)
                            <option value="{{ $loc }}" {{ old('locale') == $loc ? 'selected' : '' }}>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="excerpt[{{ $locale }}]" 
                                                  id="excerpt_{{ $locale }}" 
                                                  rows="2" 
                                                  class="form-control">{{ old("excerpt.{$locale}") }}</textarea>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="content[{{ $locale }}]" 
                                                  id="content_{{ $locale }}" 
                                                  rows="8" 
                                                  class="form-control">{{ old("content.{$locale}") }}</textarea>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="meta_description[{{ $locale }}]" 
                                                  ...
                                                  rows="2" 
                                                  class="form-control">{{ old("meta_description.{$locale}") }}</textarea>
                                    </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="template" id="template" class="form-select">
                        <option value="default" {{ old('template', 'default') == 'default' ? 'selected' : '' }}>{{ __('Default') }}</option>
                        <option value="full-width" {{ old('template') == 'full-width' ? 'selected' : '' }}>{{ __('Full Width') }}</option>
                        <option value="sidebar" {{ old('template') == 'sidebar' ? 'selected' : '' }}>{{ __('With Sidebar') }}</option>
                    </select>
                </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-4 border-top">
                ...
                <button type="submit" class="btn btn-primary btn-animated">{{ __('Create Page') }}</button>
            </div>
        </form>
    </div>
</div>
