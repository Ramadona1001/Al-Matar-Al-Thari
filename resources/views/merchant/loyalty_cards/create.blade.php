...
@section('content')
<div class="container">
  <h1 class="mb-3">{{ __('Create Loyalty Card') }}</h1>
  <form action="{{ route('merchant.loyalty-cards.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="mb-3">
    ...
    <div class="row">
      <div class="col-md-6">
        <div class="mb-3">
          ...
          <input type="number" name="points" class="form-control" min="0" value="{{ old('points', 0) }}">
        </div>
      </div>
      <div class="col-md-6">
        <div class="mb-3">
          <label class="form-label">{{ __('Balance') }}</label>
          <input type="number" name="balance" step="0.01" min="0" class="form-control" value="{{ old('balance', 0) }}">
        </div>
      </div>
    </div>
    <div class="mb-3">
    ...
    <div class="form-check mb-3">
      <input type="checkbox" name="visible_on_homepage" value="1" class="form-check-input" id="visibleHomepage">
      <label class="form-check-label" for="visibleHomepage">{{ __('Visible on homepage') }}</label>
    </div>
    <button class="btn btn-primary">{{ __('Save Card') }}</button>
